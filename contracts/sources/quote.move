module su::quote {
  ...
  public fun mint_d_coin(
    ...
    let fee_amount = mint_d_coin_fee(vault, treasury, base_in_value, base_price);
    ...
    let d_coin_amount = su_state.mint_d_coin(base_in_value) - fee_amount;

    (d_coin_amount, fee_amount, compute_fee_percent(fee_amount, base_in_value))
  } 
  ...
  public fun redeem_d_coin(
    ...
    if (d_coin_in_value == 0) return (0, 0, 0, 0);  
    ...
    let base_out = su_state.redeem(d_coin_in_value, 0, 0);
    ...
    let fee_amount = redeem_d_coin_fee(vault, treasury, base_out, base_price);
  ...
  public fun mint_d_coin_fee(
   ...
   let (max_base_in_before_stability_mode, _) = treasury.max_mintable_d_coin(
    ...
    if (max_base_in_before_stability_mode == 0)
      compute_fee(base_in_value, vault.d_stability_mint_fee())
    else {
      let standard_fee_amount = compute_fee(min(max_base_in_before_stability_mode, base_in_value), vault.d_standard_mint_fee());

      let stability_fee_amount = if (base_in_value >= max_base_in_before_stability_mode) 
        compute_fee(base_in_value - max_base_in_before_stability_mode, vault.d_stability_mint_fee())
      else 
        0;
  ...
  public fun redeem_d_coin_fee(
    ...
    if (max_base_out_before_stability_mode == 0) {
      compute_fee(base_out_value, vault.d_standard_redeem_fee())
    } else if (max_base_out_before_stability_mode >= base_out_value) {
      compute_fee(base_out_value, vault.d_stability_redeem_fee())
    } else {
      let stability_fee_amount = compute_fee(max_base_out_before_stability_mode, vault.d_stability_redeem_fee());
      let standard_fee_amount = compute_fee(base_out_value - max_base_out_before_stability_mode, vault.d_standard_redeem_fee());

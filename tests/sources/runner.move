...
module su_tests::test_runner {
  ...
  use su::sui_dollar::{Self, SUI_DOLLAR};
  ...
  public fun start(): TestRunner {
    ...
    sui_dollar::init_for_testing(ctx(scenario_mut));
    ...
    let d_treasury_cap = test_scenario::take_from_sender<TreasuryCap<SUI_DOLLAR>>(scenario_mut);
  ...
  public fun start_and_mint_all(): TestRunner {
    let mut runner = start();

    runner.next_tx(@alice);

    let (x, y, z) = mint_all(&mut runner, 100 * PRECISION, GENESIS_PRICE, 0, 0, 0);
  ...
  public fun state(self: &mut TestRunner): SuState {
    ...
    su_state::new(
      ...
      vault::last_base_nav(vault, treasury_mut),
      vault::last_f_multiple(vault, treasury_mut),
      vault::d_supply(treasury_mut),
      vault::f_supply(treasury_mut),
      vault::last_f_nav(vault, treasury_mut),
      vault::x_supply(treasury_mut),
      vault::last_x_nav(vault, treasury_mut)
    )
  }
  ...
  public fun redeem_d_coin(
    ...
    self.vault.redeem_d_coin(
      &mut self.treasury,
      &self.clock,
      coin::mint_for_testing(d_coin_in, ctx(&mut self.scenario)),
      new_price(oracle_price),
      min_base_amount,
      ctx(&mut self.scenario)
    )    
  ...
  public fun d_standard_mint_fee(self: &TestRunner): u64 {
    vault::d_standard_mint_fee(&self.vault)
  }
  ...
  public fun d_stability_mint_fee(self: &TestRunner): u64 {
    vault::d_stability_mint_fee(&self.vault)
  }
  ...
  public fun d_standard_redeem_fee(self: &TestRunner): u64 {
    vault::d_standard_redeem_fee(&self.vault)
  }

  public fun d_stability_redeem_fee(self: &TestRunner): u64 {
    vault::d_stability_redeem_fee(&self.vault)
  }  
  ...
  public fun d_supply(self: &mut TestRunner): u64 {
    vault::d_supply(&mut self.treasury)
  }
  ...
  public fun last_f_nav(self: &mut TestRunner): u64 {
    vault::last_f_nav(&self.vault, &mut self.treasury)       
  }
  ...
  public fun compute_x_nav(base_supply: u64, base_nav: u64, d_supply: u64, f_supply: u64, f_nav: u64, x_supply: u64): u64 {
    let (
      base_supply,
      base_nav,
      d_supply,
    ...
    (((base_supply * base_nav - ((f_supply * f_nav) + (d_supply * D_NAV))) / x_supply) as u64) 
  }
  ...
  public fun compute_x_value(
   ...
   let (
    ...
    base_supply * base_nav - ((f_supply * f_nav) + (d_supply * D_NAV)) 
  }

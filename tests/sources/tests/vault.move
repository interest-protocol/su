...
module su_tests::vault_tests {
  ...
  fun test_mint_all() {
    ...
    let initial_beta_coin_supply = 15 * PRECISION;
    let initial_x_coin_supply = 70 * PRECISION;
    let base_supply = 100 * PRECISION;

    let (d_coin, f_coin, x_coin) = runner.mint_all(
      base_supply,
      ORACLE_PRECISION,
      0,
      0,
    ...
    d_coin.assert_value(initial_beta_coin_supply);
    f_coin.assert_value(initial_beta_coin_supply);
    x_coin.assert_value(initial_x_coin_supply);
    ...
    .base_nav(PRECISION)
    .f_multiple(int::zero())
    .f_supply(initial_beta_coin_supply)
    .d_supply(initial_beta_coin_supply)
    .f_nav(PRECISION)                                                  
    .x_supply(initial_x_coin_supply)
    .x_nav(PRECISION);   
  ...
  #[test]
  fun test_mint_d_coin() {
    let mut runner = test_runner::start_and_mint_all();
    ...
    let initial_d_supply = runner.d_supply();

    let expected_base_in = base_in.remove_fee(runner.d_standard_mint_fee());
    ...
    let expected_d_coin_value = expected_base_in.mul_div(expected_base_nav, D_NAV);

    let expected_x_nav  = test_runner::compute_x_nav(
      initial_base_supply + expected_base_in, 
      expected_base_nav, 
      initial_d_supply + expected_d_coin_value,
      initial_f_supply, 
      expected_f_nav, 
      initial_x_supply
    );

    let d_coin = runner.mint_d_coin(base_in, price, 0);

    d_coin.assert_value(expected_d_coin_value);
    ...
    .d_supply(expected_d_coin_value + initial_d_supply)
  ...
  public fun test_redeem_d_coin() {
    ...
    let d_amount_in = 10 * PRECISION;

    let expected_i_sui_amount = math256::mul_div_down(d_amount_in.to_u256(), D_NAV.to_u256(), base_nav.to_u256());

    let i_sui = runner.redeem_d_coin(d_amount_in, price, 0);

    i_sui.assert_value(expected_i_sui_amount.to_u64().remove_fee(runner.d_standard_redeem_fee()));
  ...
  public fun test_redeem_x_coin() {
    ...
    let x_value = test_runner::compute_x_value(base_supply, base_nav, d_supply, f_supply, f_nav);

import { promises as u, existsSync as D, readFileSync as t } from "node:fs";

import i, { resolve as s } from "node:path";

import { introspectionFromSchema as F, getIntrospectionQuery as C, buildClientSchema as c, buildSchema as A, printSchema as E } from "graphql";

import { minifyIntrospection as f, outputIntrospectionFile as B, resolveTypeScriptRootDir as p } from "@gql.tada/internal";
...
function e(u, D) {
  ...
  var A = 0, E = 0, f = 0, B = (u = u || []).length;
  var p = void 0 !== (D = D || {}).alias;
  ...
  for (A = 0; A < B; A++) {
    ...
    if (0 === E) {
      ...
      F = s.substring(E, f);
      C = s.substring(++f) || A + 1 === B || 45 === ("" + u[A + 1]).charCodeAt(0) || u[++A];
      i = 2 === E ? [ F ] : F;
...
class o {
  constructor(u, D) {
    ...
    this.command(h), this.command([ v ].concat(D ? i : "<command>").join(" ")), this.single = D, 
    this.curr = "";
  }
  command(u, D, t = {}) {
    if (this.single) {
  ...
  describe(u) {
    return this.tree[this.curr || v].describe = Array.isArray(u) ? u : function(u) {
  ...
  option(u, D, t) {
    var i = this.tree[this.curr || h], [s, F] = function(u) {
      return (u || "").split(/^-{1,2}|,|\s+-{1,2}|\s+/).filter(Boolean);
    }(u);
    if (F && F.length > 1 && ([s, F] = [ F, s ]), u = "--" + s, F && F.length > 0) {
      u = `-${F}, ${u}`;
      i.alias[F] = (i.alias[F] || []).concat(s);
    }
    var C = [ u, D || "" ];
    return void 0 !== t ? (C.push(t), i.default[s] = t) : F || (i.default[s] = void 0), 
    i.options.push(C), this;
  }
  action(u) {
    return this.tree[this.curr || v].handler = u, this;
  }
  example(u) {
    return this.tree[this.curr || v].examples.push(u), this;
  }
  version(u) {
    return this.ver = u, this;
  }
  parse(u, D = {}) {
    var t, i, s, F, C = 2, c = e((u = u.slice()).slice(C), {
      alias: {
        h: "help",
        v: "version"
      }
    }), A = this.single, E = this.bin, f = "";
    if (A) {
      F = this.tree[v];
    } else {
      var B, p = 1, g = c._.length + 1;
      for (;p < g; p++) {
        if (t = c._.slice(0, p).join(" "), "string" == typeof (B = this.tree[t])) {
          i = (f = B).split(" "), u.splice(u.indexOf(c._[0]), p, ...i), p += i.length - p;
        } else if (B) {
          f = t;
        } else if (f) {
          break;
      ...
      if (s = void 0 === (F = this.tree[f])) {
        if (this.default) {
          F = this.tree[f = this.default], u.unshift(f), C++;
        } else if (t) {
          return a(E, "Invalid command: " + t);
        }
      }
    }
    if (c.help) {
      return this.help(!A && !s && f);
    }
    if (c.version) {
      return this._version();
    }
    if (!A && void 0 === F) {
      return a(E, "No command specified.");
    }
    var m = this.tree[h];
    D.alias = Object.assign(m.alias, F.alias, D.alias), D.default = Object.assign(m.default, F.default, D.default), 
    t = f.split(" "), ~(i = u.indexOf(t[0], 2)) && u.splice(i, t.length);
    var y = e(u.slice(C), D);
    if (!y || "string" == typeof y) {
      return a(E, y || "Parsed unknown option flag(s)!");
    }
    var w = F.usage.split(/\s+/), b = w.filter((u => "<" === u.charAt(0))), S = y._.splice(0, b.length);
    if (S.length < b.length) {
      return f && (E += " " + f), a(E, "Insufficient arguments!");
    }
  ...
  help(u) {
    console.log(function(u, D, t, i) {
      var s = "", F = D[t], C = "$ " + u, c = D[h], d = u => `${C} ${u}`.replace(/\s+/g, " "), A = [ [ "-h, --help", "Displays this message" ] ];
      if (t === v && A.unshift([ "-v, --version", "Displays current version" ]), F.options = (F.options || []).concat(c.options, A), 
      F.options.length > 0 && (F.usage += " [options]"), s += l("Description", F.describe, n), 
      s += l("Usage", [ F.usage ], d), i || t !== v) {
        i || t === v || (s += l("Aliases", F.alibi, d));
      } else {
        var E, f = /^__/, B = "", p = [];
        for (E in D) {
          "string" == typeof D[E] || f.test(E) || p.push([ E, (D[E].describe || [ "" ])[0] ]) < 3 && (B += `\n    ${C} ${E} --help`);
        }
        s += l("Available Commands", r(p), n), s += "\n  For more info, run any command with the `--help` flag" + B + g;
      }
      return s += l("Options", r(F.options), n), s += l("Examples", F.examples.map(d), n);
    }(this.bin, this.tree, u || v, this.single));
  }
  _version() {
...
m.exports = function() {
  ...
  var B = Math.random();
  var _uid = function(u) {
    return "Symbol(".concat(void 0 === u ? "" : u, ")_", (++f + B).toString(36));
  };
  var p = createCommonjsModule((function(t) {
  ...
  var h = p("native-function-to-string", Function.toString);
  ...
  function invalidChar(u) {
    if (void 0 === u) {
      return syntaxError("JSON5: invalid end of input at " + $ + ":" + V);
    }
    return syntaxError("JSON5: invalid character '" + formatChar(u) + "' at " + $ + ":" + V);
  }
  function invalidEOF() {
    return syntaxError("JSON5: invalid end of input at " + $ + ":" + V);
  }
  function invalidIdentifier() {
    return syntaxError("JSON5: invalid identifier character at " + $ + ":" + (V -= 5));
  }
  ...
  var K = function stringify(u, D, t) {
    ...
    if ("function" == typeof D) {
      ...
      for (var E = 0, f = D; E < f.length; E += 1) {
        ...
        if ("string" == typeof B) {
          p = B;
        } else if ("number" == typeof B || B instanceof String || B instanceof Number) {
          p = String(B);
        }
        if (void 0 !== p && F.indexOf(p) < 0) {
          F.push(p);
        }
      }
    }
  ...
  if (s) {
    ...
    if (E.ok) {
      ...
      if (f.data) {
        ...
        console.error(`Got invalid response ${JSON.stringify(f)}`);
      }
    } else {
      console.error(`Got invalid response ${await E.text()}`);
    }
  } else if ("string" == typeof t) {
    var p = ".json" === i.extname(t);
    var h = i.resolve(D, t);
    var v = await u.readFile(h, "utf-8");
    var g = p ? JSON.parse(v) : A(v);
    return "__schema" in g ? c(g) : g;
  }
};

async function generateSchema(t, {headers: F, output: A, cwd: f = process.cwd()}) {
  var B;
  try {
    B = new URL(t);
  } catch (u) {}
    ...
    if (h.ok) {
      ...
      try {
        ...
        if (g.data) {
          ...
          console.error(`Got invalid response ${JSON.stringify(g)}`);
          return;
        }
      } catch (u) {
      ...
      console.error(`Got invalid response ${await h.text()}`);
      return;
    }
  } else {
    var m = s(f, t);
    var w = await u.readFile(m, "utf-8");
    try {
      p = JSON.parse(w);
    } catch (u) {
      console.error(`Got invalid JSON ${w}`);
      return;
    }
  }
  var b = c(p);
  ...
  if (!S) {
    ...
    var N = O.compilerOptions.plugins.find((u => "@0no-co/graphqlsp" === u.name));
    S = N.schema;
    if (!N.schema.endsWith(".graphql")) {
      console.error(`Found "${N.schema}" which is not a path to a GraphQL Schema.`);
      return;
    }
  }
  await u.writeFile(s(f, S), E(b), "utf-8");
}

async function generateTadaTypes(s = !1, C = process.cwd()) {
  var c = i.resolve(C, "tsconfig.json");
  if (!D(c)) {
    console.error("Missing tsconfig.json");
    return;
  }
  var A = p(t, c) || C;
  var E = await u.readFile(i.resolve(A, "tsconfig.json"), "utf-8");
  var h;
  try {
    h = y.parse(E);
  } catch (u) {
  ...
  var v = h.compilerOptions.plugins.find((u => "@0no-co/graphqlsp" === u.name || "gql.tda/cli" === u.name));
  await async function ensureTadaIntrospection(D, t, s = process.cwd(), C = !0) {
    try {
      await (async () => {
        var c = await loadSchema(s, D);
        if (!c) {
...
export { main as default, generateSchema, generateTadaTypes };
//# sourceMappingURL=gql-tada-cli.mjs.map
